...
//			}
			...
			memberVO = memberSvc.updateMem( m_email, m_name, m_gender,m_password,  m_phone,
...
//準備進行個人密碼修改
				if ("MyfilePasswordupdate".equals(action)) {
					try {
						// 1.接收請求參數
						HttpSession session =req.getSession();
						String m_id = session.getAttribute("loginId").toString();
...
//個人密碼修改
				if ("MyfilePasswordconfirm".equals(action)) {
					List<String> errorMsgs = new LinkedList<String>();
					req.setAttribute("errorMsgs", errorMsgs);
					// 1.接收請求參數，輸入格式錯誤處理
					try {
					System.out.println("沒抓到id");	
					String m_id =new String(req.getParameter("m_id").trim());
					System.out.println("抓到id"+m_id);
					
					String m_password = req.getParameter("m_password").trim();
					System.out.println("抓到密碼");
