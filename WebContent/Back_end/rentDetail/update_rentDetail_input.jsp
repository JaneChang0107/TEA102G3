...
<%
RentDetailVO rentDetailVO = (RentDetailVO) request.getAttribute("rentDetailVO"); //EmpServlet.java (Concroller) 存入req的empVO物件 (包括幫忙取出的empVO, 也包括輸入資料錯誤時的empVO物件)
%>
...
<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/rentDetail/RentDetailServlet" name="form1">
<table>
	<tr>
	...
	<tr>
		...
		<td><select size="1" name="ro_id">
			<c:forEach var="rentOrderVO" items="${rentOrderSvc.all}">
				<option value="${rentOrderVO.ro_id}" ${(rentDetailVO.ro_id==rentOrderVO.ro_id)? 'selected':'' } >${rentOrderVO.ro_id}
			</c:forEach>
		</select></td>
	</tr>
	...
	<tr>
		...
		<td><select size="1" name="r_id">
			<c:forEach var="rentVO" items="${rentSvc.all}">
				<option value="${rentVO.r_id}" ${(rentDetailVO.r_id==rentVO.r_id)? 'selected':'' } >${rentVO.r_id}
			</c:forEach>
		</select></td>
	</tr>
	<tr>
		<td>產品數量:</td>
		<td><input type="TEXT" name="rd_count" size="45"	value="<%=rentDetailVO.getRd_count()%>" /></td>
	</tr>
...
<input type="hidden" name="rd_id" value="<%=rentDetailVO.getRd_id()%>">
