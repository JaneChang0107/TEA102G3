			...
			<div class="tab-content" id="pills-tabContent" id="myfile">
				...
				<div class="tab-pane fade show active" id="pills-home"
					...
					<table id="myfiles">
						...
						<tr>
							<td>Email</td>
							<td><p id="p_m_email">${memberVO.m_email}</p>
							<input id="input_m_email" class="-none" value="${memberVO.m_email}"></td>
                    ...
                    <input type="button" value="修改" id="revise" class="btn btn-primary">
                    </FORM>
...
$(document).on("click","#revise",function(){
	let update_name=($(this).closest("div#pills-home").find("#input_m_email").val()).trim();
	let form_data = {
			  "m_id" : '<%=session.getAttribute("loginId").toString()%>',
			  "m_email" : update_name
			  
	  };
	let form_str= JSON.stringify(form_data);
	console.log('有拿到JSON物件'+form_str);
	if(update_name == ""){
		    alert("請輸入待辦事項");
		  }else{
			  $(this).closest("div#pills-home").find("#p_m_email").text(update_name).toggleClass("-none");
			  $(this).closest("div#pills-home").find("#input_m_email").toggleClass("-none");
			  
			  if($(this).closest("div#pills-home").find("#input_m_email").hasClass("-none")){
				  $.ajax({
				        url: "http://localhost:8081/TEA102G3/MemberChangeEmail",           // 資料請求的網址
				        type: "POST",                 
				        data: form_str,                 
         				success: function(){
